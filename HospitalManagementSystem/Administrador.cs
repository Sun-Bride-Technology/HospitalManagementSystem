...
namespace HospitalManagementSystem
{
    public partial class Administrador : Form
    {
        ...
        {
            List<Medico> lstDoctores = new List<Medico>();
            foreach (var item in db.EMPLOYEE.OrderBy(c => c.Name).Where(c => c.JOB1.Position == "Medical"))
            {
                Medico doctor = new Medico()
                {
                    ...
                    ApMaterno = item.MotherLastName,
                    Edad = (int)item.Age,
                    Especialidad  = item.JOB1.Description,
                    Direccion = item.Address,
                    Telefono = item.Phone
                };
                lstDoctores.Add(doctor);
            }
            this.dgvDoctor.DataSource = lstDoctores;
        }
